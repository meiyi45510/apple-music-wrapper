...
- 宿主机侧用 `./wrapper` 管理 `build`、`login`、`2fa`、`start`、`account` 和 `version`
- 容器里运行裁剪过的 Android 用户态与 Apple Music runtime
- 对外暴露三个接口：账号信息、`m3u8` 查询、音频样本处理
...
`login` 会在终端里提示输入密码。如果账号触发 2FA，当前进程会等待 `data/2fa.txt` 出现，最多约 60 秒。另开一个终端写入验证码即可：
...
服务端会回写同长度的处理结果。发送 `sample_len = 0` 可以结束当前音轨上下文。

## 运行期数据

默认数据目录是仓库下的 `data/`，容器内映射到：
...
- [`server.c`](server.c)：登录、令牌、`m3u8`、音频处理主逻辑
...
- `dev_token`、`music_token` 和处理后的数据都应该被当成敏感资产处理。
